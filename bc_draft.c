...
void subtraction(list* ptr1, list* ptr2,list *diff){
	...
	/*while(diff->front->val==0){
		if(diff->front->next==NULL) 
		break;
		deleteLeft(diff);
		diff->front=diff->front->next;
	}*/
}	
/*void binarySubtraction(list* ptr1, list* ptr2, list* diff){
	node* p1;
	node* p2;
	if(isgreater(ptr1,ptr2)==0){
		insertLeft(diff,0);
	...
	/*printf("result of binary subtraction: ");
	displayLR(diff);
	while(diff->front->val==0){
		if(diff->front->next==NULL) 
		break;
...
/*void divisiongpt(list* ptr1, list* ptr2, list* quotient) {
	printf("enters divisiongpt\n");
    list temp;
    init(&temp);
    ...
    while (ptr1->front != NULL) {
        ...
        while (isgreater(ptr1, ptr2)==1) {
            subtraction(ptr1, ptr2, &temp);
            ++digit;
            // Copy the result back to ptr1 for the next iteration
            node* current = ptr1->front;
            node* tempCurrent = temp.front;
            while (current && tempCurrent) {
                current->val = tempCurrent->val;
                current = current->next;
                tempCurrent = tempCurrent->next;
            }
        }

        insertRight(quotient, digit);
...
/*
list* decimalToBinary(list* ptr1){
	list* bin=(list*)malloc(sizeof(list));
	init(bin);
	node* p=ptr1->rear;
	while(){
		*/
